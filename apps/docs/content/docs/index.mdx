...
- 🚀 **Deploy instantly** - Plugin updates available immediately to all consumers
- 📦 **Zero conflicts** - Shared dependencies via Module Federation
- 🔄 **Independent versioning** - No coordinated releases required
- ⚡ **No build cycles** - Just deploy and use
...
```typescript
const { client, router, metadata } = await runtime.usePlugin("plugin-id", config);
...
```typescript
// Production - Load from CDN via Module Federation
const runtime = createPluginRuntime({
  registry: {
    "plugin-id": {
...
// Development - Load locally without Module Federation
import { createLocalPluginRuntime } from "every-plugin/testing";
const runtime = createLocalPluginRuntime(
  { registry: {...} },
  { "plugin-id": PluginImplementation } as const
);
...
│   ├── service.ts     # Plain class wrapping external API
│   └── index.ts       # Plugin implementation
├── rspack.config.cjs  # Build config (uses EveryPluginDevServer)
└── plugin.dev.ts      # Dev server configuration
```
