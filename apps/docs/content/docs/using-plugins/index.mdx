...
const runtime = createPluginRuntime({
  registry: {
    "data-source": {
      remote: "https://cdn.example.com/plugins/data-source/remoteEntry.js"
    },
    "transformer": {
      remote: "https://cdn.example.com/plugins/transformer/remoteEntry.js"
    }
  },
  secrets: {
...
## Organizing Runtime for Multi-File Applications

For applications with multiple entry points (main process + workers, multiple services, etc.), create a dedicated runtime module to share plugin instances:

```typescript
// plugins.ts - Single source of truth for plugin initialization
import type MyPlugin from "@org/my-plugin";
import { createPluginRuntime } from "every-plugin/runtime";

// Module augmentation for type safety across your app
declare module "every-plugin" {
  interface RegisteredPlugins {
    "@org/my-plugin": typeof MyPlugin;
  }
}
...
export const runtime = createPluginRuntime({
  registry: {
    "@org/my-plugin": {
      remote: "https://cdn.example.com/plugins/my/remoteEntry.js"
    }
  },
  secrets: {
    API_KEY: process.env.API_KEY!
...
const myPlugin = await runtime.usePlugin("@org/my-plugin", {
  secrets: { apiKey: "{{API_KEY}}" },
  variables: { timeout: 30000 }
});
...
const { client } = plugins.myPlugin;  // Already initialized
...
const { client } = plugins.myPlugin;  // Same instance as main.ts
const result = await client.workerTask(task.data);
```

**Benefits:**
- ✅ Plugin initialized once, reused everywhere
- ✅ Type safety via module augmentation
- ✅ No duplicate initialization in workers
- ✅ Clean separation of setup from business logic
...
```typescript
const { createClient, router, metadata } = await runtime.usePlugin("data-source", config);
...
function logPluginUsage(plugin: DataSourcePlugin) {
  console.log(`[${plugin.metadata.id}]`);
}
...
## Worker & Multi-Process Integration

For complex applications, integrate with job processing systems. Combine with the shared runtime pattern for best results:
...
const queue = new Queue('plugin-jobs', { connection: redisConnection });
...
const worker = new Worker(
  ...
  async (job: Job) => {
    ...
    try {
      const { client } = plugins.myPlugin;  // Shared, pre-initialized plugin
      return await client.process(taskData);
    } catch (error) {
...
// Queue job from main process
await queue.add('process', { taskData: { id: "123" } });
...
```typescript
try {
  const { createClient } = await runtime.usePlugin("data-source", config);
  const client = createClient();
