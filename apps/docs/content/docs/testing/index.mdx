...
The every-plugin framework provides testing utilities to test plugins without Module Federation overhead.

## Testing Utilities

Import the testing helpers from `every-plugin/testing`:

```typescript
import { createTestPluginRuntime } from "every-plugin/testing";
import type { TestPluginMap } from "every-plugin/testing";
import { Effect } from "every-plugin/effect";
import { expect, it } from "@effect/vitest";
```

## Type Bindings for Testing

Type bindings are **optional** but provide significant benefits for testing:
...
To use type bindings in tests, import the plugin type-only:

```typescript
// Install plugin as dev dependency (or workspace dependency)
// bun add -D @my-org/my-plugin

import type { PluginBinding } from "every-plugin";
import type TestPlugin from "./my-plugin"; // Type-only import

type TestBindings = {
  "my-plugin": PluginBinding<typeof TestPlugin>;
};
```

Type bindings are completely optional - your tests work without them, but they enhance your development experience.

## Unit Testing

Test plugins in isolation using `createTestPluginRuntime`:

```typescript
import { createTestPluginRuntime } from "every-plugin/testing";
import type { TestPluginMap, PluginBinding } from "every-plugin";
import TestPlugin from "./my-plugin";

// Optional: Define type bindings for autocomplete
type TestBindings = {
  "my-plugin": PluginBinding<typeof TestPlugin>;
};

// Create plugin map (actual implementation)
const pluginMap: TestPluginMap = {
  "my-plugin": TestPlugin
};

// Create test runtime
const { runtime, PluginService } = createTestPluginRuntime<TestBindings>(
  {
    registry: {
      "my-plugin": {
