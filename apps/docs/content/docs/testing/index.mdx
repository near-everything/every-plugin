...
Test plugins using `createPluginRuntime` with module entries, which loads plugins directly without Module Federation overhead.

## Setup

Import the runtime and your plugin:
...
<Callout>
Module entries provide automatic type inference from the plugin constructor. No manual type bindings needed!
</Callout>
...
it("should produce events in background", async () => {
  const { createClient } = await runtime.usePlugin("background-plugin", {
    secrets: { apiKey: "{{API_KEY}}" },
    variables: {
...
describe("Plugin Integration", () => {
  const runtime = createPluginRuntime({
    registry: {
      "source": { module: SourcePlugin },
      "transformer": { module: TransformerPlugin }
    },
    secrets: { API_KEY: "test-key" }
  });

  it("should process data through pipeline", async () => {
    const { createClient: createSource } = await runtime.usePlugin("source", {
      secrets: { apiKey: "{{API_KEY}}" }
    });
...
describe("Complete Plugin Test Suite", () => {
  ...
  describe("Basic Operations", () => {
    ...
    it("should fetch by ID", async () => {
      const { createClient } = await runtime.usePlugin("test-plugin", TEST_CONFIG);
      const client = createClient();
      const result = await client.getById({ id: "test-123" });
