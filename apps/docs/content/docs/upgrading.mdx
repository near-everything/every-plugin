...
Version 0.7.0 introduces a `createClient` factory pattern that enables per-request context injection for authentication middleware and request-specific data.

### What's New

- 🔐 **Context Injection** - `createClient({ userId: "..." })` for per-request context
- 🛡️ **Authentication Middleware** - Middleware can access request context for auth checks
- 🏭 **Factory Pattern** - `createClient()` factory replaces direct `client` property
- 📝 **Context Schema** - Plugins declare expected context shape with Zod

### Breaking Changes

#### 1. usePlugin API Changes
...
const client = createClient(); // or createClient({ userId: "..." })
const result = await client.getData({ id: "123" });
```

**Changes:**
- `client` property replaced with `createClient` factory function
- Factory accepts optional context parameter for authentication
- All existing functionality preserved - just new API surface

#### 2. Plugin Context Schema

Plugins can now declare expected request context using Zod schemas:

```typescript
export default createPlugin({
  // NEW: Context schema declaration
  context: z.object({
...
Context is validated and passed to route handlers via oRPC's context system. This enables type-safe middleware that can access request-specific data like user authentication, session information, or permissions.

#### 3. Middleware Access to Context

Middleware can now access request context for authentication:
  ...
  return {
    protectedRoute: builder.protectedRoute
      ...
      .handler(async ({ input, context }) => {
        // context.userId is now guaranteed non-null
        return { data: "protected", userId: context.userId };
      })
  };
}
```
...
#### Step 1: Update usePlugin Calls

Replace all `const { client } = ...` with `const { createClient } = ...; const client = createClient();`:

```typescript
// Before
const { client } = await runtime.usePlugin("my-plugin", config);
await client.getData({ id: "123" });

// After
const { createClient } = await runtime.usePlugin("my-plugin", config);
const client = createClient(); // For public routes
await client.getData({ id: "123" });

// For authenticated routes
const authClient = createClient({ userId: "user123" });
await authClient.getProtectedData({ id: "123" });
```

#### Step 2: Update Plugin Definitions (Optional)

If you want to add authentication middleware, declare a context schema:
...
export default createPlugin({
  ...
  // Update middleware to use context
  createRouter: (deps, builder) => {
    ...
    return {
      ...
      privateRoute: builder.privateRoute
        .use(requireAuth)
        .handler(async ({ input, context }) => {
          // context.userId is available and typed
          return { data: "private", userId: context.userId };
        })
    };
  }
});
...
All test files need to be updated to use the factory pattern:

```typescript
// Before
const { client } = await runtime.usePlugin("test-plugin", config);
...
// For context tests
const authClient = createClient({ userId: "test-user" });
const protectedResult = await authClient.getProtectedData({ id: "test" });
```

### Backwards Compatibility

- All existing plugins continue to work without changes
- The factory pattern is backward-compatible for public routes
- Context schema is optional - plugins without it work normally
- Middleware without context access continues to work

### Benefits

- **Authentication Support**: Middleware can access user context for auth decisions
- **Per-Request Context**: Different contexts for different users/sessions
- **Type Safety**: Context is fully typed and validated
- **Flexible**: Context can include user ID, session data, permissions, etc.
...
- 🐛 **Module-Only Plugin Fix** - Plugins with direct module imports no longer error with undefined remote URLs
- 🔧 **Rspack 1.6.x Support** - Fixed Module Federation compatibility with rspack 1.6.x
- 📦 **Simplified Template** - Merged tsconfig files, updated package.json exports
- 🧹 **Cleaner Dependencies** - Removed unused dependencies (msw, sirv) from template
...
- 🔄 **Unified API** - Single `createPluginRuntime` for both module and remote loading
- 🎯 **Automatic Type Inference** - Types derived from `module` entries without augmentation
- 📦 **Flexible Registry** - Mix module and remote entries in the same configuration
- 🏷️ **Simplified Fields** - `remoteUrl` → `remote`, removed `version` field
- 🔧 **Enhanced EveryPlugin.Infer** - Now accepts runtime instances for better type inference
...
- 🏗️ **Module Augmentation** - Declare plugins for full type safety
- 📂 **Multi-Plugin Pattern** - Shared plugin instances across files
- 🔧 **Type Inference** - `EveryPlugin.Infer<K>` for extracting plugin types
- 🔗 **Plugin Bindings** - Automatic type safety via static binding property
...
const { createClient } = plugins.dataSource;  // Already initialized
const client = createClient();
