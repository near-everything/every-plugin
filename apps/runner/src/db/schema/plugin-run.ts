...
export const pluginRunStatusEnum = pgEnum(
	"plugin_run_status",
	pluginRunStatusValues,
);
export const pluginRunTypeEnum = pgEnum("plugin_run_type", [
	"SOURCE",
	"PIPELINE",
]);

export const pluginRun = pgTable(
	"plugin_run",
	{
		...
		sourceItemId: varchar("source_item_id", { length: 255 }).references(
			() => sourceItem.id,
			{ onDelete: "cascade" },
		),
