...
const program = Effect.scoped(
  Effect.gen(function* () {
    ...
    yield* Effect.fork(createSourceWorker.pipe(
      Effect.provide(PluginRuntimeLive)
    ));
    yield* Effect.fork(createPipelineWorker.pipe(
      Effect.provide(PluginRuntimeLive)
    ));

    yield* Effect.log("✅ Application fully started (HTTP server + scheduler + workers)");
    yield* Effect.never;
  })
).pipe(
  Effect.catchAll((error) =>
