...
export const EnvironmentServiceTag =
	Context.GenericTag<EnvironmentService>("EnvironmentService");
	...
	return {
		hydrateSecrets: <T>(
			...
			Effect.gen(function* () {
				...
				yield* validateRequiredSecrets(templateVars, availableSecretNames);

				const view: Record<string, any> = {};
				for (const templateVar of templateVars) {
					if (
						availableSecretNames.includes(templateVar) &&
						envMapping[templateVar]
					) {
				...
				const populatedSecretsString = Mustache.render(
					stringifiedSecrets,
					view,
				);
