...
export const queueRouter = {
	...
	deleteJob: authenticatedProcedure
		...
		.handler(async ({ input, context }) => {
			...
			const program = Effect.gen(function* () {
				...
				return {
					success: true,
					data: { message: `Job ${jobId} has been deleted.` },
				};
			});
	...
	resumeQueue: authenticatedProcedure
		...
		.handler(async ({ input, context }) => {
			...
			const program = Effect.gen(function* () {
				...
				return {
					success: true,
					data: { message: `Queue ${queueName} has been resumed.` },
				};
			});
	...
	pauseQueue: authenticatedProcedure
		...
		.handler(async ({ input, context }) => {
			...
			const program = Effect.gen(function* () {
				...
				return {
					success: true,
					data: { message: `Queue ${queueName} has been paused.` },
				};
			});
	...
	clearQueue: authenticatedProcedure
		...
		.handler(async ({ input, context }) => {
			...
			const program = Effect.gen(function* () {
				const queueService = yield* QueueService;
				const result = yield* queueService.clearQueue(
