...
import type { ExecutePipelineJobData } from '@usersdotfun/shared-types/types';
...
const processPipelineJob = (job: Job<ExecutePipelineJobData>) =>
  Effect.gen(function* () {
    ...
    let startIndex = 0;
    if (startAtStepId) {
      const stepIndex = workflow.pipeline.steps.findIndex(s => s.stepId === startAtStepId);
      if (stepIndex === -1) {
        return yield* Effect.fail(new Error(`Step ${startAtStepId} not found in pipeline`));
      }
      startIndex = stepIndex;
      yield* Effect.log(`Starting pipeline from step ${startAtStepId} (index ${startIndex})`);
    }
    ...
    for (const stepDefinition of stepsToProcess) {
      ...
      const existingPluginRun = yield* workflowService.getPluginRunByStep(workflowRunId, sourceItemId, stepDefinition.stepId).pipe(
      ...
      if (existingPluginRun && existingPluginRun.status === 'PENDING') {
        ...
        pluginRun = yield* workflowService.updatePluginRun(existingPluginRun.id, {
          status: 'RUNNING',
          output: null,
          error: null,
          completedAt: null,
        });
      } else if (existingPluginRun && existingPluginRun.status === 'COMPLETED') {
    ...
    const runItems = yield* workflowService.getItemsForWorkflowRun(workflowRunId);
    ...
    if (processedCount === runItems.length && runItems.length > 0) {
      ...
      yield* Effect.log(`Workflow run ${workflowRunId} completed - all ${runItems.length} items processed`);
    }
    ...
    Effect.catchAll(error =>
      Effect.gen(function* () {
        ...
        const currentRun = yield* workflowService.getWorkflowRunById(workflowRunId!);
