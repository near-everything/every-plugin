...
export function getStructuredDataScript(metadata: SiteMetadata): HeadScript {
  return {
...
export function getRemoteScriptsWithMetadata(
  options: RemoteScriptsOptions,
  ...
  return [
    getRemoteEntryScript(assetsUrl),
    getThemeInitScript(),
    getStructuredDataScript(metadata),
    getHydrateScript(runtimeConfig, containerName, hydratePath),
  ];
}
