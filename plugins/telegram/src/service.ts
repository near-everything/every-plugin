...
function handleTelegramError(error: unknown, errors: {
...
export class TelegramService {
  ...
  createBot() {
    ...
    return Effect.gen(function* () {
      ...
      yield* Effect.tryPromise({
        ...
        catch: (error: unknown) => new Error(`Bot token validation failed: ${error instanceof Error ? error.message : String(error)}`)
      });
  ...
  setupPollingMiddleware(bot: Telegraf, queue: Queue.Queue<Context<Update>>) {
    return Effect.gen(function* () {
      ...
      yield* Effect.sync(() => console.log("[Telegram] Polling middleware setup complete"));
    });
  }
