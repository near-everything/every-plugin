...
describe("Telegram Message Logic Tests", () => {
	...
	describe("Plugin Initialization", () => {
		it(
			"should initialize plugin successfully with polling mode",
			async () => {
	...
	describe("Message Sending", () => {
		...
		it(
			"should send message with formatting",
			async () => {
				const { client } = await runtime.usePlugin(
					"@curatedotfun/telegram",
					SHARED_TEST_CONFIG,
	...
	describe("Webhook Processing Logic", () => {
		it(
			"should process webhook updates without registering webhook",
			async () => {
				const { client } = await runtime.usePlugin(
					"@curatedotfun/telegram",
					SHARED_TEST_CONFIG,
				...
				// Create test updates
				const update1 = createTextUpdate(
					"Hello webhook!",
	...
	describe("Stream Processing Logic", () => {
		it(
			"should process queued updates and stream them",
			async () => {
				const { client } = await runtime.usePlugin(
					"@curatedotfun/telegram",
					SHARED_TEST_CONFIG,
				...
				const update1 = createTextUpdate(
					"Stream test message 1",
					parseInt(TEST_CHAT_ID),
				);
				const update2 = createTextUpdate(
				...
				console.log(
					`✅ Stream test completed with ${contexts.length} contexts`,
				);
				...
				const textContexts = contexts.filter(
					(ctx) =>
						ctx.message &&
						"text" in ctx.message &&
						(ctx.message.text?.includes("Stream test message") ||
		...
		it(
			...
			async () => {
				...
				const targetUpdate = createTextUpdate(
					"Target chat message",
					parseInt(TEST_CHAT_ID),
				);
				...
				console.log(
					`✅ Chat ID filter test completed with ${contexts.length} contexts`,
				);
		...
		it(
			"should filter commands using messageTypes",
			async () => {
				const { client } = await runtime.usePlugin(
					"@curatedotfun/telegram",
					SHARED_TEST_CONFIG,
				...
				// Send mixed messages
				const regularUpdate = createTextUpdate(
					"Regular message",
					parseInt(TEST_CHAT_ID),
				);
				const commandUpdate = createCommandUpdate(
					"/command",
					parseInt(TEST_CHAT_ID),
				);
				...
				console.log(
					`✅ Message type filter test completed with ${contexts.length} contexts`,
				);
		...
		it(
			"should handle stream completion properly",
			async () => {
				const { client } = await runtime.usePlugin(
					"@curatedotfun/telegram",
					SHARED_TEST_CONFIG,
				...
				console.log(
					`✅ Stream completion test finished with ${events.length} events`,
				);
