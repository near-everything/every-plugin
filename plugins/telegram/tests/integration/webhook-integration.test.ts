...
const createWebhookUpdateFromSentMessage = (
	sentMessage: { messageId: number },
	content: string,
) => ({
...
describe.sequential("Telegram Webhook Integration Tests", () => {
	...
	it(
		"should handle complete integration flow with all features",
		async () => {
			...
			console.log(
				"🚀 Testing complete integration: API → webhook → listen → stream",
			);
			...
			console.log(
				`✅ Test 1: Sent real message ${sentMessage.messageId} via Telegram API`,
			);

			// Simulate webhook for the message
			const webhookUpdate = createWebhookUpdateFromSentMessage(
				sentMessage,
				testMessage,
			);
			...
			console.log(
				`✅ Test 1: Processed webhook update for message ${sentMessage.messageId}`,
			);
			...
			for (const message of filterMessages) {
				...
				const filterWebhookUpdate = createWebhookUpdateFromSentMessage(
					result,
					message,
				);
			...
			console.log(
				`✅ Test 2: Sent and processed ${filterMessages.length} messages via webhook`,
			);
			...
			const formats = [
				...
				{
					text: "<b>HTML Bold integration test</b>",
			...
			console.log(
				`✅ Test 3: Successfully sent ${formats.length} formatted messages`,
			);
			...
			console.log(
				`✅ Test 4: Reply functionality verified: reply ${replyResult.messageId} to message ${originalResult.messageId}`,
			);
			...
			await Promise.race([
				(async () => {
					for await (const ctx of streamResult) {
						console.log(
							`🔍 Received message via stream: Update ${ctx.update?.update_id}`,
						);
			...
			if (events.length > 0) {
				...
				console.log(
					"ℹ️ No events received - this may be expected if queue was already consumed",
				);
			}
			...
			console.log(
				`🎉 Integration test suite complete! Confirmation message ${confirmationResult.messageId} sent`,
			);
		},
		{ timeout: 30000 },
	);
